module calc_parameter
  ...
  subroutine calc_srat_rperm(num, pertur, pres, srat, rperm, sstor, wstor)
  !*********************************************************************************************
  ...
  !*********************************************************************************************
    ...
    ! -- local
    integer(I4) :: i
    real(DP) :: retm, beta, theta, kr, se, ss, ws
    real(DP) :: per_phead, phead0
    real(DP) :: kr_phead, kr_beta, kr_se
    !-------------------------------------------------------------------------------------------
    phead0 = DZERO

    !$omp parallel do private(i, per_phead, retm, beta, theta, kr, se, ss, ws) &
